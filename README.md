...
この例では音符を 42 個推定し、正解譜面も 42 個です。しかし、推定音高が全体に 1 オクターブ高いため、開始時刻 ±50 ms・音高 ±50 セントで採点した補正前の音符 F1 は **0%** です。原因を切り分けるため、推定音高を一律 12 半音下げて採点すると **85.7%** になります。
...
現行の Soramimic Score で開発用の日本語 J-POP 9 曲を解析し、同じ音源に対応する XF 譜面のメロディ音符と比較しました。9 曲とも解析を完了し、正解音符は計 5,814 個です。音符の F1 は全曲の音符をまとめて計算し、開始時刻が ±50 ms 以内、音高が ±50 セント以内なら一致とします。「終了も一致」では、終了時刻の誤差が 50 ms または正解音長の 20% の大きい方以内であることも求めます。
...
音源と譜面の一定の時刻差は曲ごとに補正し、音高は補正していません。歌詞の文字誤り率は、空白と句読点を除いた表記の編集距離から計算しました。開発中に結果を確認した曲なので、未知の曲に対する性能を保証する値ではありません。
...
| 開始時刻・音高の許容差 | Score の音符 F1 | STARS の音符 F1 | STARS を 1 オクターブ上げた診断値 |
| --- | ---: | ---: | ---: |
| ±50 ms・±50 セント | 76.8% | 0.3% | 21.0% |
| ±100 ms・±1 半音 | 78.8% | 0.4% | 27.7% |
| ±200 ms・±1 半音 | 83.0% | 0.5% | 35.9% |

時刻差は両方式で同じ手順により曲ごとに補正しています。STARS の出力音高は XF 譜面よりおおむね 1 オクターブ低く、右列は原因を切り分けるための診断値です。実際の出力精度は中央列です。STARS は今回、Score の認識歌詞を受け取っており、独立した歌詞認識の比較ではありません。公開モデルは日本語を対象としていないため、この結果から両方式の一般的な優劣は判断できません。

### GTSinger での移植元の参考比較

別の [GTSinger](https://github.com/AaronZ345/GTSinger) 日本語歌唱 24 クリップでも、移植元の自動解析と [STARS](https://github.com/gwx314/STARS) を比較しました。STARS には移植元が認識した歌詞の読みを渡し、音符の時刻や音高は渡していません。両方式とも音符を出せなかった 1 クリップは予測なしとして 24 クリップの集計に含めています。音符と歌詞の対応を同時に求める F1 は次の通りです。

| 開始時刻・音高の許容差 | 移植元の音符 F1 | STARS の音符 F1 | 移植元の歌詞込み F1 | STARS の歌詞込み F1 |
| --- | ---: | ---: | ---: | ---: |
| ±50 ms・±50 セント | 26.6% | 33.2% | 17.6% | 16.7% |
| ±100 ms・±1 半音 | 54.0% | 49.4% | 37.7% | 24.2% |
| ±200 ms・±1 半音 | 63.8% | 58.9% | 47.7% | 28.8% |

移植元の認識歌詞を正解と比べると、文字の一致率は **67.7%**、カナの一致率は **72.6%**、母音の一致率は **64.1%** でした。文字・カナは正解列の長さ、母音は比較する 2 列の長い方を分母とする編集距離から計算しています。母音の正解は GTSinger の音素表記から復元しており、解釈できない音素は比較から除いています。

表の移植元の値は、出力音高を一律 1 オクターブ下げて採点した**診断値**です。補正前の実際の出力精度や現行の Score の精度ではありません。また、[STARS の論文](https://aclanthology.org/2025.findings-acl.781/)で学習に使った GTSinger は中国語・英語部分で、今回の日本語部分ではありません。公開モデルも日本語を対象としていないため、この表は両者の一般的な優劣を示しません。
