...
現行の Soramimic Score で開発用の日本語 J-POP 9 曲を解析し、同じ音源に対応する歌詞付き譜面のメロディ音符と比較しました。9 曲とも解析を完了し、正解音符は計 5,814 個です。音符の F1 は全曲の音符をまとめて計算し、開始時刻が ±50 ms 以内、音高が ±50 セント以内なら一致とします。「終了も一致」では、終了時刻の誤差が 50 ms または正解音長の 20% の大きい方以内であることも求めます。採点時は、音域のずれに対して方式ごとに一律のオクターブ移動を許容します。
...
音符の開始時刻・音高に加え、その音符に付いたモーラの読みも一致した正解音符の割合は次の通りです。読みは「トー」と「トオ」のように同じ発音になる表記をそろえて比較し、1 音符に複数のモーラが付く場合は全体の読みが一致したときだけ数えます。

| 歌詞の入力 | 3 条件が一致した正解音符 | 余分な推定音符も含めた F1 |
| --- | ---: | ---: |
| 自動認識 | 68.9%（4,007 / 5,814） | 66.8% |
| 正解歌詞を指定 | 70.7%（4,112 / 5,814） | 68.8% |

この値は音符の検出とモーラの割り当てを合わせた結果です。音符の開始時刻または音高が外れた場合も不一致になるため、モーラの割り当てだけの正答率ではありません。
...
同じ開発用 J-POP 9 曲から各曲の 60〜90 秒を切り出し、歌詞付き譜面の計 806 音符と比較しました。Score の自動認識から得た同じ音素列を、[STARS](https://github.com/gwx314/STARS) の公開モデルにも渡しています。STARS には、同じ音素列を発音に近いローマ字に直し、分離したボーカルに別の CTC モデルで整列して得た時刻と長さも渡し、原曲入力と分離ボーカル入力の両方を試しました。譜面の音符や時刻は推論へ渡していません。

| 方法・入力音声 | 音符の開始・音高 F1 | モーラも一致する F1 |
| --- | ---: | ---: |
| Score | 83.2% | 71.5% |
| STARS・原曲 | 8.1% | 5.0% |
| STARS・分離ボーカル | 23.3% | 13.9% |

「モーラも一致する F1」は、開始時刻 ±50 ms、音高 ±50 セント、音符に付いた読みのすべてが一致する条件です。譜面との一定の時刻差は全方式で同じ値を使って採点し、STARS の音高は一律 12 半音上げています。STARS に時刻を渡さない場合も試しましたが、両音源とも推定音符は 9 区間すべてで同一でした。時刻の指定は、この実装では主に音符と読みの対応に影響しました。

この比較は各曲 30 秒の開発用データについての値で、上のフル曲 9 曲の集計とは対象区間が異なります。STARS の公開モデルは日本語を学習対象としていないため、両方式の一般的な優劣を示すものではありません。
