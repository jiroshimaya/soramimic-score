...
flowchart TD
    ...
    I --> S["入力歌詞があれば認識結果と対応付け<br/>確かな箇所の表示を入力表記にする"]
    D["入力歌詞（任意）"] --> S
    S --> T["読みが異なる行は音声で再確認<br/>支持された行だけ局所的に再整列"]
    T --> J["歌詞・読み・時刻・音高・<br/>信頼度と判断根拠をJSONに保存"]
    J -.-> K["カラオケ表示"]
    J -.-> L["XF MIDI"]
    J -.-> M["MusicXML"]
    J -.-> N["字幕"]
...
ある場合も自動認識を行い、対応が確かな箇所の表示に入力表記を使います。
...
まずWhisperの認識結果から、通常どおり読み・時刻・音符を推定します。
その後に入力歌詞を対応付け、表記と歌唱データを分けて保持します。
一対一で対応した行では、推定済みの読みと`soramimic-yomi`・UniDic Liteの候補を
音声と照合します。読みが変わる行だけを再整列し、他の行の歌唱データを変えてしまう
候補は採用しません。改行をまたぐ対応や音声の根拠が曖昧な箇所は、自動解析の読みを維持します。
...
このオプションは既定の表記対応付けとは異なり、補正後の歌詞を使って読みと時刻を
推定し直します。認識ミスで入力歌詞を失う可能性があるため、既定はオフです。
...
読みの再確認結果を保存します。`canonical`は歌唱データに対応する認識表記を保持し、
文字単位の時刻を推測して入力表記へ割り当てることはしません。
