...
現行の Soramimic Score で開発用の日本語 J-POP 9 曲を解析し、同じ音源に対応する歌詞付き譜面のメロディ音符と比較しました。9 曲とも解析を完了し、正解音符は計 5,814 個です。音符の F1 は全曲の音符をまとめて計算し、開始時刻が ±50 ms 以内、音高が ±50 セント以内なら一致とします。「終了も一致」では、終了時刻の誤差が 50 ms または正解音長の 20% の大きい方以内であることも求めます。採点時は、音源と譜面の一定の時刻差を曲ごとに補正し、音域のずれに対して方式ごとに一律のオクターブ移動を許容します。
...
同じ 9 曲で、評価用の読みと比べたカナ・母音の一致率は次の通りです。カナは「キャ」のような拗音を 1 個とするモーラ単位で比べ、「ン」「ー」「ッ」も含めます。母音は各モーラから取り出した a・i・u・e・o を比べ、「ン」「ー」「ッ」は正解と推定の両方から除きます。どちらも正解列に対する編集距離で計算します。
