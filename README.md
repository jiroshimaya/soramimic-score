...
同じ開発用 J-POP 9 曲の全曲を、歌詞付き譜面の計 5,814 音符と比較しました。[STARS](https://github.com/gwx314/STARS) の公開モデルには、分離したボーカルと Score が自動認識した発音列を渡しています。外部で整列した発音時刻や譜面の音符は渡していません。

| 方法 | 音符の開始・音高 F1 | モーラも一致する F1 |
| --- | ---: | ---: |
| Score | 76.8% | 66.8% |
| STARS | 18.3% | 0.6% |

「モーラも一致する F1」は、開始時刻 ±50 ms、音高 ±50 セント、音符に付いた読みのすべてが一致する条件です。譜面との一定の時刻差は両方式で同じ値を使って採点し、STARS の音高は一律 12 半音上げています。STARS の読みは、モデルが推定した語の時刻から音符に対応付けました。

STARS の公開モデルは日本語を学習対象としていないため、この結果は両方式の一般的な優劣を示すものではありません。
