...
現行の Soramimic Score で開発用の日本語 J-POP 9 曲を解析し、同じ音源に対応する XF 譜面のメロディ音符と比較しました。9 曲とも解析を完了し、正解音符は計 5,814 個です。音符の F1 は全曲の音符をまとめて計算し、開始時刻が ±50 ms 以内、音高が ±50 セント以内なら一致とします。「終了も一致」では、終了時刻の誤差が 50 ms または正解音長の 20% の大きい方以内であることも求めます。採点時は、音域のずれに対して方式ごとに一律のオクターブ移動を許容します。

| 歌詞の入力 | 音符の開始・音高 F1 | 終了も一致する F1 | 歌詞の文字誤り率 |
| --- | ---: | ---: | ---: |
| 自動認識 | 76.8% | 61.5% | 13.4% |
| 正解歌詞を指定 | 76.9% | 61.5% | 3.3% |

同じ 9 曲で、評価用の読みと比べたカナ・母音の一致率は次の通りです。カナは「キャ」のような拗音を 1 個とするモーラ単位で比べ、「ン」「ー」「ッ」も含めます。母音は各モーラから取り出した a・i・u・e・o を比べ、「ン」「ー」「ッ」は正解と推定の両方から除きます。どちらも正解列に対する編集距離で計算します。複数の読みが成り立つ語も、評価用の読み 1 種類と比べています。
...
音源と譜面の一定の時刻差は曲ごとに補正しています。歌詞の文字誤り率は、空白と句読点を除いた表記の編集距離から計算しました。開発中に結果を確認した曲なので、未知の曲に対する性能を保証する値ではありません。

### 同じ J-POP 9 曲での STARS 比較

[STARS](https://github.com/gwx314/STARS) の公開モデルにも同じ 9 曲を入力し、同じ XF 譜面で音符を採点しました。STARS には分離したボーカルと、Score が自動認識した発音列を渡しています。日本語の音素は公開モデルの語彙に変換しました。正解譜面の音高・時刻は推論へ渡していません。

| 開始時刻・音高の許容差 | Score の音符 F1 | STARS の音符 F1 |
| --- | ---: | ---: |
| ±50 ms・±50 セント | 76.8% | 21.0% |
| ±100 ms・±1 半音 | 78.8% | 27.7% |
| ±200 ms・±1 半音 | 83.0% | 35.9% |

時刻差は両方式で同じ手順により曲ごとに補正しています。STARS は今回、Score の認識歌詞を受け取っており、独立した歌詞認識の比較ではありません。公開モデルは日本語を対象としていないため、この結果から両方式の一般的な優劣は判断できません。注：STARS の音高は採点時に一律 12 半音上げ、Score の音高は移動していません。

### GTSinger 日本語歌唱での評価

[GTSinger](https://huggingface.co/datasets/GTSinger/GTSinger) の日本語歌唱から、2 人の歌手と 6 種類の歌唱条件について、通常歌唱と技法歌唱を各 1 クリップ選びました。難しいクリップを選別したものではありません。重複しない計 24 クリップを歌詞なしで現行の Soramimic Score に入力し、全件で解析が完了しました。正解音符は計 407 個です。

同じ 24 クリップを [STARS](https://github.com/gwx314/STARS) の公開モデルでも解析し、全件で完了しました。STARS には歌唱音源と Score が認識した発音列を渡し、正解音符の時刻・音高は渡していません。両方式の音符を同じ正解音符と許容差で採点した結果です。

| 開始時刻・音高の許容差 | Score の音符 F1 | STARS の音符 F1 |
| --- | ---: | ---: |
| ±50 ms・±50 セント | 27.8% | 27.2% |
| ±100 ms・±1 半音 | 58.4% | 45.8% |
| ±200 ms・±1 半音 | 67.8% | 55.3% |

認識歌詞の文字一致率は **82.9%**、カナ一致率は **82.6%**、母音一致率は **87.8%** でした。各一致率は正解列の長さを分母とする編集距離で計算し、カナ・母音における「ン」「ー」「ッ」の扱いは上の J-POP 評価と同じです。カナ・母音の参照列は GTSinger の表記歌詞を `soramimic-yomi` で読みに変換したもので、人手で確認した発音正解ではありません。

J-POP 9 曲の値とは音源・歌手・正解譜面が異なり、J-POP では曲ごとの一定の時刻差を補正しています。この 24 クリップでは時刻を補正していません。通常歌唱 12 クリップと技法歌唱 12 クリップの Score の厳しい条件での F1 は、それぞれ 28.2% と 27.4% で、大きな差は見られません。参考までに正解音符の開始時刻を使ってクリップごとの一定の時刻差を補正すると、Score の F1 は 40.3% になります。両データの数字だけで GTSinger の歌唱が J-POP より難しいとは判断できません。

注：採点時に Score の音高を一律 12 半音下げ、STARS の音高は移動していません。STARS は Score の認識した発音列を受け取っており、独立した歌詞認識の比較ではありません。[STARS の論文](https://aclanthology.org/2025.findings-acl.781/)では GTSinger の中国語・英語部分を使用していますが、今回の日本語部分は対象外で、公開モデルも日本語を学習対象としていません。
