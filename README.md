...
flowchart TD
    ...
    D --> F["辞書から歌詞の読みを生成<br/>UniDic Lite"]
    E --> F
...
分離しています。モデルの重みは同梱しません。ボーカル分離、音声による読み候補の
選び直し、XF MIDI・MusicXML・字幕への書き出しは、この標準構成には含まれません。
...
WhisperとReazonSpeechは初回実行時に取得します。取得済みのモデルだけを使う場合は
